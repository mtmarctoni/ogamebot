...
def navigate_to_section(page: Page, planet_id: PlanetId, section: COMPONENTS) -> None:
    ...
    if section == COMPONENTS.SUPPLIES:
        ...
        navigate_to_research_page(page, planet_id)
        print(f"[DEBUG] Navigated to research section of planet ID {planet_id}.")
    else:
        raise ValueError(f"Unknown section: {section}")

def navigate_to_resources_page(page: Page, planet_id: PlanetId) -> None:
    """
...
def navigate_to_lifeform_page(page: Page, planet_id: PlanetId) -> None:
    """
...
def navigate_to_research_page(page: Page, planet_id: PlanetId) -> None:
    """
    Navigates to the research page of the specified planet.
    ...
    Returns:
        None
    """
    url = COMPONENT_URL_TEMPLATE.format(component=COMPONENTS.RESEARCH.value, planet_id=planet_id)
    if not url.startswith("http"):
        raise ValueError(f"Generated URL is invalid: {url}")

    print(f"[DEBUG] Navigating to research page for planet_id: {planet_id}")
    print(f"[DEBUG] Generated URL: {url}")
    ...
    for attempt in range(retries):
        try:
            ...
            if not page.url.startswith(url):
                raise RuntimeError(f"Failed to navigate to the research page for planet_id {planet_id}.")
