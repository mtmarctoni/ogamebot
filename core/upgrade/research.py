...
def handle_research_upgrades(planet: PlanetDict, page: Page, config: ConfigType, notifier: Optional[TelegramNotifier] = None) -> List[int]:
    ...
    for research in prioritized_researches:
        research_id = Researches.get_id_by_name(research.name)
        research_info = planet['research'][research_id]
        current_level = research_info['level']
        cap = research_soft_caps[research.value]
        ...
        if research_info.get('upgradable', False):
            ...
            print(f"[DEBUG] Upgrading {research} on planet {planet['name']} ({planet['coords']}).")
            duration = upgrade_tech(**params)

            if duration > 0:
                upgrade_durations.append(duration)
                safe_notify(notifier, f"Upgraded {research} on planet {planet['name']} ({planet['coords']}).. Duration: {duration} seconds.")
            else:
                safe_notify(notifier, f"Failed to upgrade {research} on planet {planet['name']} ({planet['coords']})..")
